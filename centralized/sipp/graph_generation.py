...
class State(object):
    def __init__(self, position=(-1, -1), t=0, interval=(0, float('inf'))):
        ...
        self.pos_inr = (position, interval)
    ...
    def init_graph(self):
        for i in range(self.dimensions[0]):
            for j in range(self.dimensions[1]):
                ...
                self.sipp_position_graph.update(grid_dict)

    def init_intervals(self):
        if not self.dyn_obstacles:
            for key, value in self.sipp_position_graph.items():
                grid_dict = {(key, (0, float('inf'))): SippGrid()}
        ...
        for schedule in self.dyn_obstacles.values():
            ...
            for i in range(len(schedule)):
                ...
                self.sipp_position_graph[position].split_interval(t, last_t)

                for inr in self.sipp_position_graph[position].interval_list:
                    print((position, inr))
                    grid_dict = {(position, inr): SippGrid()}
                    self.sipp_graph.update(grid_dict)
